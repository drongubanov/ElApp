...
/**
 * Ток в нейтральном (нулевом рабочем) проводнике при несимметричной нагрузке
 * трёх фаз. Для трёх токов, сдвинутых на 120° (одинаковый характер нагрузки по
 * фазам), геометрическая сумма фазных токов, возвращающихся через нейтраль:
 *   Iн = √(I1² + I2² + I3² − I1·I2 − I2·I3 − I3·I1).
 * При равных токах Iн = 0 (симметрия), при нагрузке только на одной фазе Iн = Iф.
 * Высшие гармоники (в т.ч. 3-я от нелинейных нагрузок), складывающиеся в
 * нейтрали, этой формулой не учитываются — это оценка по основной гармонике.
 */
export function neutralCurrent(i1, i2, i3) {
  const sum = i1 * i1 + i2 * i2 + i3 * i3 - i1 * i2 - i2 * i3 - i3 * i1;
